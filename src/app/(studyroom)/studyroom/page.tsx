    ...
    <>
      ...
      <div className="flex-1 bg-muted p-4 pb-10">
        ...
        <Link href={'/study/write'}>
          ...
          {joinedStudyList.length > 0 && 
            <div>
              <p className='font-semibold text-lg pb-3'>참여중인 스터디</p>
              <div className='bg-white flex flex-col border rounded-lg [&>*:first-child]:border-none'>
                {joinedStudyList.map((study: any) => (
                  <Link href={`./studyroom/${study.id}/calendar`} className='border-t' key={study.id}>
                    <div className='flex gap-3 p-2'>
                      <StudyAvatar />
                      <p className='flex flex-col text-start'>
                        <span className='font-semibold'>{study.title}</span>
                        <span className='font-medium text-xs'>{study.tags}</span>
                      </p>
                    </div>
                  </Link>
                ))}
